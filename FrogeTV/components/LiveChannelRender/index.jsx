...
function LiveChannelRender() {
    ...
    const liveChannelsList = useSelector((state) => state.search.liveChannelsList);
    ...
    const fetchCategories = useCallback(async () => {
        ...
        setIsFetching(true);
        try {
            const newCursor = await dispatch(searchActions.getLiveChannels(cursor));
            if (newCursor) {
                setCursor(newCursor);
            }
        } finally {
    ...
    useEffect(() => {
        const fetchInitialCategories = async () => {
            //if (isFocused) {
                setIsFetching(true);
                try {
                    const newCursor = await dispatch(searchActions.getLiveChannels(null));
                    if (newCursor) {
                        setCursor(newCursor);
                    }
                } finally {
    ...
    const onRefresh = useCallback(async () => {
        setRefreshing(true);
        const newCursor = await dispatch(searchActions.getStreamsfromCategories(null));
    ...
    return (
        <DynamicFlatList
            ...
            contentContainerStyle={{ padding: 0, marginLeft: "auto", marginRight: "auto" }}
            refreshControl={
                <RefreshControl
...
export default LiveChannelRender;
